...
from utils.repo_archive import archive_repository
...
class PredictionEngine:
    ...
    def analyze(self, request: AnalyzeRepositoryRequest) -> AnalysisResult:
        ...
        try:
            ...
            # Generate reports fully in-memory — no files written to disk.
            json_bytes, pdf_bytes = self.report_generator.generate(result)
            result.json_bytes = json_bytes
            result.pdf_bytes = pdf_bytes
            result.repo_archive_bytes = archive_repository(repository_path)
            result.repository_path = f"db://{analysis_id}"
            return result
        except Exception as exc:
            raise AnalysisError(f"Repository analysis failed: {exc}") from exc
        finally:
            # Always remove the temporary clone directory.
            if tmp_dir:
                self._safe_rmtree(Path(tmp_dir))
